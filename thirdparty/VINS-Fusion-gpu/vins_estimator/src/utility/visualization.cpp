...
{
    // The offline adapter runs without a ROS master: the static
    // TransformBroadcaster below constructs a ros::NodeHandle on first use,
    // and ros::start() enters an endless XMLRPC retry loop against the master.
    // TF broadcast is irrelevant to the adapter's TUM-file contract, so skip
    // it entirely.
    (void)estimator;
    (void)header;
    return;
